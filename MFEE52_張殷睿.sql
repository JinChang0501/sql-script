...
-- 計算票數量
select ticket, max(ticket) from full_passengers
group by ticket;



select name,survived, max((select count(ticket) from full_passengers where pname like 'Andersson, %')) from full_passengers
group by ticket;

select ticket, count(ticket) from full_passengers
group by ticket;
